...
<Code.Description>
  提供六种基础类型的按钮使用。搭配 `outline`
  属性，你将再获得额外五种类型的按钮。此外，还有 `link` 按钮。
</Code.Description>
...
<Code.Description>
  添加跳转链接。若设置此属性，button的表现将跟 `a` 标签一致。
</Code.Description>
...
| icon      | 设置按钮图标，位于文字左侧（如果有文字的话）               | `React.ReactNode`                                                      | -         |
| href      | 添加跳转链接，设置此属性，button 的表现将跟 `a` 标签一致。 | `string`                                                               | -         |
| target    | 作为 `a` 标签的 target 属性。仅当 href 存在时生效          | `HTMLAttributeAnchorTarget`                                            | -         |
